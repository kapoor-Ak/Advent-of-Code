...
safe_flag <- function(report){
  ...
  # or just could have used the normal diff() function
  #monotonous check
...
part2_func <- function(report){
  any(map(seq_along(report),\(x) report[-x] |> safe_flag()) ==1)
}

part_1 <- map(input[!is.na(input)],\(x) safe_flag(x)) |> unlist() |> sum()
#606
part_2 <-map(input[!is.na(input)],\(x) part2_func(x)) |> unlist() |> sum()
#644
